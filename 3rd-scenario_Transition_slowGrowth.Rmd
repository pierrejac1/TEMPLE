---
title: "TEMPLE model: simulation for the third scenario (energy transition and slow economic growth)"
...
intermediateVar <- c(EROI_nre = "1 + A*exp(-B*cum_y_nre)",
                     ...
                     uc_e = "min(omega_e*gamma_e/u_e + epsilon_e + gamma_e*delta_e/(P_e*u_e), 0.5391434)", # unit cost of energy production is capped thanks to government subsidies in order to cap inflation in the energy price
...
uc_e = (uc_e < 0.5391434) * uc_e + (uc_e >= 0.5391434) * 0.5391434 # unit cost of energy production is capped thanks to government subsidies in order to cap inflation in the energy price
p_hat_e = beta_pe * (mu_e*uc_e + T_e0 - 1)
pi_e = P_e*u_e/gamma_e * (1-uc_e-T_e0) - 0.01*d_e # The debt of energy firms is guaranteed by the state so that they benefit from an interest rate of 1%
pi_f = K_e*(k_hat_e+delta_e) + k_hat_f + delta_f + kappa_hf + kappa_cf - u_f/gamma_f*uc_f - r*d_f
upsilon_f = beta_nf1 * (gamma_f*(K_e*(k_hat_e+delta_e) + k_hat_f + delta_f + kappa_hf + kappa_cf + delta_n*nu_f) - u_f_e)
...
matplot(year, k_hat_f, type="l", xlab="Time", ylab="", main="Growth rate of capital stock of final goods firms")
